git # Copyright 2022 Huawei Technologies Co., Ltd
#
...
from src.models.misc import DropPath1D, Identity
...
class ConvBlock(nn.Cell):
    ...
    def __init__(self, inplanes, outplanes, stride=1, res_conv=False, act_layer=nn.ReLU, group=1,
                 norm_layer=partial(BatchNorm2d, eps=1e-6), drop_block=None, drop_path=None, return_x_2=False):
        super(ConvBlock, self).__init__()

        expansion = 4
        med_planes = outplanes // expansion

        self.conv1 = nn.Conv2d(inplanes, med_planes, kernel_size=1, stride=1, pad_mode="valid", has_bias=False)
        self.bn1 = norm_layer(med_planes)
        self.act1 = act_layer()

        self.conv2 = nn.Conv2d(med_planes, med_planes, kernel_size=3, stride=stride, group=group, pad_mode='pad', padding=1, has_bias=False)
        self.bn2 = norm_layer(med_planes)
        self.act2 = act_layer()

        self.conv3 = nn.Conv2d(med_planes, outplanes, kernel_size=1, stride=1, pad_mode="valid", has_bias=False)
        self.bn3 = norm_layer(outplanes)
        self.act3 = act_layer()

        if res_conv:
            self.residual_conv = nn.Conv2d(inplanes, outplanes, kernel_size=1, stride=stride, pad_mode="valid", has_bias=False)
...
class FCUDown(nn.Cell):
    ...
    def __init__(self, inplanes, outplanes, dw_stride, act_layer=nn.GELU, norm_layer=partial(nn.LayerNorm, epsilon=1e-6)):
...
class FCUUp(nn.Cell):
    ...
    def __init__(self, inplanes, outplanes, up_stride, act_layer=nn.ReLU,
        ...
        self.conv_project = nn.Conv2d(inplanes, outplanes, kernel_size=1, stride=1, padding=0, has_bias=True) # bias ?
...
class ConvTransBlock(nn.Cell):
    ...
    def __init__(self, inplanes, outplanes, res_conv, stride, dw_stride, embed_dim, num_heads=12, mlp_ratio=4.,
        ...
        expansion = 4
        self.cnn_block = ConvBlock(inplanes=inplanes, outplanes=outplanes, res_conv=res_conv, stride=stride, group=group, return_x_2=True)

        if last_fusion:
            self.fusion_block = ConvBlock(inplanes=outplanes, outplanes=outplanes, stride=2, res_conv=True, group=group)
        else:
            self.fusion_block = ConvBlock(inplanes=outplanes, outplanes=outplanes, group=group)
...
class Conformer(nn.Cell):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=1000, base_channel=64, channel_ratio=4, num_med_block=0,
        ...
        self.trans_dpr = np.linspace(0, drop_path_rate, depth).tolist()  # stochastic depth decay rule
        ...
        self.conv1 = nn.Conv2d(in_chans, 64, kernel_size=7, stride=2, padding=3, pad_mode='pad', has_bias=False)  # 1 / 2 [112, 112]
        ...
        self.conv_trans = nn.CellList()
        # 2~4 stage
        init_stage = 2
        fin_stage = depth // 3 + 1
        for i in range(init_stage, fin_stage):
            self.conv_trans.append(ConvTransBlock(stage_1_channel, stage_1_channel, False, 1, dw_stride=trans_dw_stride, embed_dim=embed_dim,
                                                  num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                                                  qk_scale=qk_scale, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, drop_path_rate=self.trans_dpr[i - 1],
        ...
        for i in range(init_stage, fin_stage):
            ...
            self.conv_trans.append(ConvTransBlock(
